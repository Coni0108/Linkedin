...
#####
{
  ...
  base$fina= toupper(base$fina)
  base$fina= iconv(base$fina, to="ASCII//TRANSLIT")
  base$inicioa=toupper(base$inicioa)
  base$inicioa= iconv(base$inicioa, to= "ASCII//TRANSLIT")
}
#####
...
##### 
#ELIMINAR ESPACIOS EN "POSICION"
{
  puesto= gsub(pattern = "\\s",   
               replacement = "",
               x = employ$posicion)
  employ$puesto= puesto
  employ$posicion=NULL
}

ues = ues %>% sort(table(educ$institucion), decreasing=TRUE)
RANKING=head(ues, 5)
View(RANKING) #RANKING UNIVERSIDADES
barplot.default(RANKING, main = "TOP 5 UNIVERSIDADES", col = c(2,5,8,7,3))
...
educ= educ %>%
  filter(ae>=0 & ae<=16)
View(mibase)
summary(mibase) #resumen de los años de educación

{
  data1= educ %>%
    group_by(exec_id) %>%
    summarise(sum(ae))
  View(data1) #data de años de educación por persona
  summary(data1)
